...
@section('content')
	...
	<div class="card mb-4">
		...
		<form class="card-body" action="{{ route('certificate_save', $course->id) }}" method="POST" enctype="multipart/form-data">
			@csrf
			...
			<div class="row g-3">
				...
				<div class="col-6">
					...
					<input value="{{ old('username') }}" name="username" type="text" id="username" class="form-control @error('username') is-invalid @enderror" placeholder="{{ trans('global.username') }}" />
				</div>
				<div class="col-6">
					<label class="form-label " for="score">{{ trans('global.score') }}</label>
					<input value="{{ old('score', 0) }}" name="score" type="number" id="score" class="form-control @error('score') is-invalid @enderror" placeholder="{{ trans('global.score') }}" />
				</div>
						...
						<div class="col-md mb-md-0 mb-2">
							<div class="form-check custom-option custom-option-basic">
								<label class="form-check-label custom-option-content pt-1 pb-1"
									...
									<input name="status" @if(old('status')==="1" ) checked @endif
						...
						<div class="col-md">
							<div class="form-check custom-option custom-option-basic">
								<label class="form-check-label custom-option-content pt-1 pb-1"
									...
									<input name="status" @if(old('status')==="0" ) checked @endif
