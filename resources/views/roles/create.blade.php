...
@section('title', trans("global.create_role"))
...
@section('content')
	...
	@php
		$links = [
			...
			"end" => trans("global.create_role"),
		]
	@endphp
	@include("layouts.breadcrumbs")
	<form class="card-body row" id="save_role" action="{{route('role_save')}}" method="POST">
		@csrf
		<div class="col-12">
			<div class="card mb-4">
				<h5 class="card-header">@lang("global.create_role")</h5>
				<div class="card-body">
					<div>
						<label for="name" class="form-label">{@lang("global.name")</label>
						<input type="text" value="{{ old("name") }}" class="form-control" id="name" name="name" placeholder="@lang("global.create_role")" aria-describedby="defaultFormControlHelp" />
					</div>
					<!-- Project table -->
					<div class="card my-4">
						<div class="card-body">
							...
							<table class="table table-striped border-top">
									...
									<tr checkedvalue="0">
										...
										<td>
											<div class="form-check d-flex justify-content-center">
												<input class="form-check-input showValue" type="checkbox" name="permission[]" value="{{$permission[0]->id}}" checked />
											</div>
										</td>
										<td>
											<div class="form-check d-flex justify-content-center">
												<input class="form-check-input createValue" type="checkbox" name="permission[]" value="{{$permission[1]->id}}" checked />
											</div>
										</td>
										<td>
											<div class="form-check d-flex justify-content-center">
												<input class="form-check-input editValue" type="checkbox" name="permission[]" value="{{$permission[2]->id}}" checked />
											</div>
										</td>
										<td>
											<div class="form-check d-flex justify-content-center">
												<input class="form-check-input deleteValue" type="checkbox" name="permission[]" value="{{$permission[3]->id}}" checked />
								...
								@foreach($permissions as $key => $permission)
									<tr>
										...
										<td>
											<div class="form-check d-flex justify-content-center">
												<input class="form-check-input showValue" type="checkbox" name="permission[]" value="{{$permission->id}}" id="defaultCheck10" checked />
											</div>
										</td>
									</tr>
								@endforeach
