...
@section('title', trans('global.create_ask'))
...
@section('content')
	@php
		$links = [
			...
			"end" => trans("global.create_ask"),
	]
	@endphp
	@include("layouts.breadcrumbs")

	<form class="card-body row" action="{{route('ask_save')}}" method="POST" enctype="multipart/form-data">
		@csrf
		<div class="col-12">
			@if ($errors->any())
			...
			@endif
			<div class="card mb-4">
				<h5 class="card-header">@lang("global.create_ask")</h5>
				<div class="card-body">
					<div class="row mb-3">
						<div class="col-6">
							<label for="title_ar" class="form-label">@lang("global.title_ar")</label>
							<input type="text" value="{{ old("title_ar") }}" class="form-control @error('title_ar') is-invalid @enderror" id="title_ar" name="title_ar" placeholder="@lang("global.title_ar")" aria-describedby="defaultFormControlHelp" />
						</div>
						<div class="col-6">
							<label for="title_en" class="form-label">@lang("global.title_en")</label>
							<input type="text" value="{{ old("title_en") }}" class="form-control @error('title_en') is-invalid @enderror" id="title_en" name="title_en" placeholder="@lang("global.title_en")" aria-describedby="defaultFormControlHelp" />
						</div>
