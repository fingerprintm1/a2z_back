			...
			<div class="row g-3">
				...
				<div class="col-4 ">
					...
					<select id="section_id" name="section_id" class="select2 form-select form-select-lg section_id @error('section_id') is-invalid @enderror" data-allow-clear="true">
						<option selected value="">@lang("global.choose_section")</option>
						@foreach ($sections as $section)
							<option value="{{ $section->id }}" @if (old('section_id', $user->section_id) == $section->id) selected @endif>{{ $section["name_". app()->getLocale()] }}</option>
						@endforeach
					</select>
				</div>
				<div class="card-body col-md-4 p-0 mt-3 pe-2 ps-2">
